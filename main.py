...
def prepare_data():
    ...
    std_scaler = StandardScaler(withMean=False, withStd=True).fit(feature_RDD)
...
def draw_graph(train_data, validation_data):
    lambda_param_list = [1.0, 3.0, 5.0, 15.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 60.0]
    my_metrics = [
        train_evaluation_model(train_data, validation_data, lambda_param)
        for lambda_param in lambda_param_list
    ...
    df = pd.DataFrame(my_metrics,
                      index=list(range(len(lambda_param_list))),
                      columns=['AUC', 'duration', 'lambdaParam', 'model'])
    show_chart(df, "lambdaParam", 'AUC', 'duration', 0.5, 0.7)
...
if __name__ == "__main__":
    ...
    draw_graph(train_d, validation_d)
